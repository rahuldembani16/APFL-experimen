...
from Models import Mnist_2NN, Mnist_CNN, cifar10_cnn, PlantVillage_CNN
...
# 数据集
parser.add_argument('-dataset', "--dataset", type=str, default="plantvillage", help="需要训练的数据集")
...
# ckks设置
# 优化CKKS参数以支持更大的模型
context = ts.context(ts.SCHEME_TYPE.CKKS, poly_modulus_degree=16384, coeff_mod_bit_sizes=[60, 40, 40, 40, 40, 60])
...
if __name__ == "__main__":
    ...
    # ResNet网络
        ...
        net = cifar10_cnn()
    elif args['model_name'] == 'plantvillage_cnn':
        net = PlantVillage_CNN()
    # 如果有多个GPU
    ...
    if args['dataset'] == 'cifar10' or args['dataset'] == 'plantvillage':
...
for i in range(args['num_comm']):
    ...
    data.to_csv(log_filename, mode='a', header=False, index=False)  # mode设为a,就可以
